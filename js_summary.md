...
не видны в CoffeeMachine. Как недорешение: условилиться протектед-члены делать
как публичные, но имя начинается с _
...
Помимо простого обращения `obj.\_\_proto\_\_` есть методы
...
Аналогично: `setInterval`/`clearInterval`. Тут учитывается время
 _между_стартами_функций_ кроме IE, который считает время между концом прошлой и стартом следующей
...
Новый вариант объявления функции: **=>**.
