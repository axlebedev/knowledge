...
Можно так: каждая линия оканчивается на `\`. Но делать так не стоит: магия с
пробелами в начале/конце строки, и такое не входит в стандарт ЕС.
ПЛОХО:
```JavaScript
var ssstring = "lalala \
                bebebe";
...
В `alert()`, бинарном `+`, где один из агрументов - строка.  
Явно делается так:
```JavaScript
String(null); //null
...
В математических операциях, в операторах сравнения (кроме `===` и `!==`).
Явно вызывается унарным `+`, либо `Number(val)`.
```JavaScript
+'123'; //123
+undefined; //NaN
+null; //0
+true; +false; //1, 0
```
Не забываем, что `null == undefined`.  

**Логическое преобразование**  
В `if`, `while`, `for`.  
Явно вызывается `!!` или `Boolean(val)`.
```JavaScript
...
не только по цифровым индексам. Использовать обычный обход `for(var i = 0; i < arr.length; ++i)`  
...
```
Удалять поля через `delete` не рекомендуется, очень медленная операция (изменение количества полей в объекте медленно).
Лучше просто присвоить полю какой-нибудь `null` .
...
Если не-_writable_ property будет слева от присваивания - то без `"use strict"` ничего не произойдет,
...
function Rabbit() {
    ...
    me.constructor = Rabbit; //Не пропустить эту строчку
    return me;
}
```
